...
TextLabel4.Text = "if you see this then the script may have overloaded"
...
for i,v in pairs(game:GetService("Players"):GetPlayers()) do
  spawn(function()
	v.Chatted:Connect(function(mes)
		if not string.sub(mes, 1, 1) == "@" then return end
		local ping = returnargument(mes)
		if not ping then return end
		if ping == "@everyone" or ping == "@here" then
		for i,p in pairs(game:GetService("Players"):GetPlayers()) do
			chat(p, v, mes)
			end
    elseif ping == "@someone" then
    local rd = random()
    chat(rd, v, mes)
		end
		local player = find(string.sub(ping, 2))
