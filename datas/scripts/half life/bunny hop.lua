-- bhop script (not) by pdn [ marseio ]
-- if you dont know to to bhop, learn them: https://www.youtube.com/watch?v=wDBL2pvkmUk
print("some bhop script that i found while screwing around with free model and converted to p1 by pdn")
print("if you dont know how to bhop, learn them first\nhttps://www.youtube.com/watch?v=wDBL2pvkmUk <- how2bhop video")
local player
local character
local collider
local camera
...
function main()
	...
	sinit(player, UDim2.new(0.9, -100, 0.5, 50))
	--connect updateloop
  game:GetService("RunService").RenderStepped:Connect(updateLoop)
...
function updateLoop()
	setDeltaTime();
	update(updateDT);
	supdate(player.Character.HumanoidRootPart.Velocity);
end
function sinit(player, position)
	local gui = Instance.new("ScreenGui", script)
	display = Instance.new("TextLabel", gui)
